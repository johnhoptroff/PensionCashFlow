...
class TestDecayofNetWorthModerateInflation {
	...
	void test() {
		...
		LocalDate dateEnd = LocalDate.of(2058, 12, 31);
		LocalDate dateStart = LocalDate.of(2024, 1, 1);
		double dNetWorth = CalcNetWorth.calcTotalWorth(accountsJohn) + CalcNetWorth.calcTotalWorth(accountsLynne);
		System.out.println("Initial NetWorth:" + dNetWorth);
		SpendingProfile spTarget = new SpendingProfile(dNetWorth,dInflation,dateStart,dateEnd);
		CashFlow cashFlow = new CashFlow(People,spTarget,LocalDate.of(2024,1,1),txParams,niParams);
		try {
			cashFlow.runMaxEarnings(LocalDate.of(2058,12,31));
			assertEquals(0.0, cashFlow.getdInflation(),0.1); //was 2058
		} catch (Exception e) {
